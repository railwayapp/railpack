...
func TestFilterExcludedFiles(t *testing.T) {
	t.Run("drops files excluded by dockerignore", func(t *testing.T) {
		ctx := CreateTestContext(t, "../../examples/dockerignore")

		filtered := ctx.FilterExcludedFiles([]string{
			"package.json",
			"the.log",
			"Justfile",
			"app/README.md",
		...
		require.Equal(t, []string{"package.json", "app/README.md"}, filtered)
	})

	t.Run("drops files under an excluded directory", func(t *testing.T) {
		ctx := CreateTestContext(t, "../../examples/dockerignore")

		// `**/node_modules` excludes the directory, so everything beneath it
		// is absent from the build context too
		filtered := ctx.FilterExcludedFiles([]string{
			"web/node_modules/left-pad/package.json",
			"web/package.json",
		})

		require.Equal(t, []string{"web/package.json"}, filtered)
	})
	...
	t.Run("honours negation patterns", func(t *testing.T) {
		...
		filtered := ctx.FilterExcludedFiles([]string{
			...
			"negation_test/excluded.txt",
		})

		require.Equal(t, []string{"negation_test/should_exist.txt"}, filtered)
	})

	t.Run("applies config exclude patterns", func(t *testing.T) {
		ctx := CreateTestContext(t, "../../examples/node-npm")
		ctx.Config.Exclude = []string{"docs/"}
		...
		require.Equal(t, []string{"package.json"}, filtered)
	})

	t.Run("returns everything when nothing is excluded", func(t *testing.T) {
		ctx := CreateTestContext(t, "../../examples/node-npm")

		paths := []string{"package.json", "docs/marketing/package.json"}
		require.Equal(t, paths, ctx.FilterExcludedFiles(paths))
	})
}
