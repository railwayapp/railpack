...
func (p *NodeProvider) applyNodeVersionSources(ctx *generate.GenerateContext, miseStep *generate.MiseStepBuilder, node resolver.PackageRef) {
	...
	if nvmrc, err := ctx.App.ReadFile(".nvmrc"); err == nil {
		...
		miseStep.Version(node, nvmrc, ".nvmrc")
	}

	if nodeVersionFile, err := ctx.App.ReadFile(".node-version"); err == nil {
		miseStep.Version(node, nodeVersionFile, ".node-version")
	}
}
...
func (p *NodeProvider) InstallMisePackages(ctx *generate.GenerateContext, miseStep *generate.MiseStepBuilder) {
	...
	if requiresNode {
		...
		p.applyNodeVersionSources(ctx, miseStep, node)
	}
	...
	if p.requiresBun(ctx) {
		...
		if !requiresNode && ctx.Config.Packages["node"] == "" {
			node := miseStep.Default("node", "latest")
			misePackages = append(misePackages, "node")
			p.applyNodeVersionSources(ctx, miseStep, node)
