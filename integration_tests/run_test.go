	...
	for _, entry := range entries {
		...
		for i, testCase := range testCases {
			...
			t.Run(testName, func(t *testing.T) {
				...
				if err := runContainerWithTimeout(t, imageName, testCase.ExpectedOutput, testCase.Envs, testCase.Platform, networkName); err != nil {
					t.Fatal(err)
				}
			})
		}
...
func runContainerWithTimeout(t *testing.T, imageName string, expectedOutputs []string, envs map[string]string, platformStr string, networkName string) error {
